Lumen::App.controllers do
  ...
  get '/digest' do
    ...
    @hot_conversations = current_account.visible_conversations.where(:updated_at.gte => @from).where(:updated_at.lt => @to+1).order_by(:updated_at.desc).select { |conversation| conversation.visible_conversation_posts.count >= (ENV['HOT_CONVERSATION_THRESHOLD'] || 3) }
