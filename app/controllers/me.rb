Lumen::App.controller do
  ...
  get '/me/review' do
    ...
    @new_events = current_account.events.where(:created_at.gte => @from).where(:created_at.lt => @to+1).where(:start_time.gte => @from).order_by(:start_time.asc)
    @period_events = current_account.events.where(:start_time.gte => @from).where(:start_time.lt => @to+1).order_by(:start_time.asc)
