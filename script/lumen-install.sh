...
15 * * * * /usr/bin/dokku run $APP_NAME rake groups:test_creating_a_conversation_via_email
EOT

SESSION_SECRET=$(uuidgen)
DRAGONFLY_SECRET=$(uuidgen)
dokku config:set --no-restart $APP_NAME APP_NAME=$APP_NAME DOMAIN=$DOMAIN MAIL_DOMAIN=$MAIL_DOMAIN MAIL_SERVER_ADDRESS=$MAIL_SERVER_ADDRESS MAIL_SERVER_USERNAME=root MAIL_SERVER_PASSWORD=$MAIL_SERVER_PASSWORD SESSION_SECRET=$SESSION_SECRET DRAGONFLY_SECRET=$DRAGONFLY_SECRET
